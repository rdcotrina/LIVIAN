    ...
    public function post($arrDecrypt = []) {
        ...
        foreach ($_POST as $key => $value) {
            if (is_array($_POST[$key])) {
                ...
                $this->_objectForm->$key = $arr;
                $this->_decryptArray($arrDecrypt, $_POST[$key], $key);
            } else {
                $this->_objectForm->$key = htmlspecialchars(trim($value), ENT_QUOTES);
                $this->_decrypt($arrDecrypt, $key, $value);
            }
        }
    ...
    public function get($arrDecrypt = []) {
        ...
        foreach ($_GET as $key => $value) {
            if (is_array($_GET[$key])) {
                ...
                $this->_objectForm->$key = $arr;
                $this->_decryptArray($arrDecrypt, $_GET[$key], $key);
            } else {
                $this->_objectForm->$key = htmlspecialchars(trim($value), ENT_QUOTES);
                $this->_decrypt($arrDecrypt, $key, $value);
            }
    ...
    public function file($arrDecrypt = []) {
        ...
        foreach ($_FILES as $key => $value) {
            if (is_array($_FILES[$key])) {
                ...
                $this->_objectForm->$key = $arr;
                $this->_decryptArray($arrDecrypt, $_FILES[$key], $key);
            } else {
                $this->_objectForm->$key = htmlspecialchars(trim($value), ENT_QUOTES);
                $this->_decrypt($arrDecrypt, $key, $value);
            }
    ...
    public function request($arrDecrypt = []) {
        ...
        foreach ($_REQUEST as $key => $value) {
            if (is_array($_REQUEST[$key])) {
                ...
                $this->_objectForm->$key = $arr;
                $this->_decryptArray($arrDecrypt, $_REQUEST[$key], $key);
            } else {
                $this->_objectForm->$key = htmlspecialchars(trim($value), ENT_QUOTES);
                $this->_decrypt($arrDecrypt, $key, $value);
            }
